...
/* !< Function pointer parsing one argument at argv[index], returning the number of parsed
 * arguments, or a negative value when the argument is invalid */
 ...
 */
void SDLCALL SDLTest_CommonLogUsage(
    SDLTest_CommonState* state, const char* argv0, const char** options);
 ...
 */
SDL_AppResult SDLCALL SDLTest_CommonEventMainCallbacks(
    SDLTest_CommonState* state, const SDL_Event* event);
 ...
 *
 */
void SDLCALL SDLTest_CommonDrawWindowInfo(
    SDL_Renderer* renderer, SDL_Window* window, float* usedHeight);
