...
static void Render(SDL_Window* window, const int windownum)
{
    ...
    if (!SDL_AcquireGPUSwapchainTexture(
            cmd, state->windows[windownum], &swapchainTexture, &drawablew, &drawableh)) {
...
static SDL_GPUShader* load_shader(bool is_vertex)
{
    ...
    if (format & SDL_GPU_SHADERFORMAT_DXIL) {
        ...
        createinfo.code_size
            = is_vertex ? SDL_arraysize(D3D12_CubeVert) : SDL_arraysize(D3D12_CubeFrag);
...
static void init_render_state(int msaa)
{
    ...
    if (msaa
        && SDL_GPUTextureSupportsSampleCount(gpu_device,
