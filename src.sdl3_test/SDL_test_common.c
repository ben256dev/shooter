...
static const char* common_usage[] = { "[-h | --help]", "[--trackmem]", "[--randmem]",
    "[--info all|video|modes|render|event|event_motion]",
    "[--log all|error|system|audio|video|render|input]", NULL };

static const char* video_usage[] = { "[--always-on-top]", "[--aspect min-max]",
    "[--auto-scale-content]", "[--center | --position X,Y]", "[--confine-cursor X,Y,W,H]",
    "[--depth N]", "[--display N]", "[--flash-on-focus-loss]",
    "[--fullscreen | --fullscreen-desktop | --windows N]", "[--geometry WxH]", "[--gldebug]",
    "[--grab]", "[--hidden]", "[--hide-cursor]", "[--high-pixel-density]", "[--icon icon.bmp]",
    "[--input-focus]", "[--keyboard-grab]",
    "[--logical-presentation disabled|match|stretch|letterbox|overscan|integer_scale]",
    "[--logical-scale-quality nearest|linear|best]", "[--logical WxH]", "[--max-geometry WxH]",
    "[--maximize]", "[--metal-window | --opengl-window | --vulkan-window]", "[--min-geometry WxH]",
    "[--minimize]", "[--mouse-focus]", "[--noframe]", "[--refresh R]", "[--renderer driver]",
    "[--resizable]", "[--scale N]", "[--title title]", "[--transparent]", "[--usable-bounds]",
    "[--utility]", "[--video driver]", "[--gpu driver]", "[--vsync]", NULL };

/* !!! FIXME: Float32? Sint32? */
static const char* audio_usage[] = { "[--audio driver]", "[--rate N]",
    "[--format U8|S8|S16|S16LE|S16BE|S32|S32LE|S32BE|F32|F32LE|F32BE]", "[--channels N]", NULL };

static void SDL_snprintfcat(
    SDL_OUT_Z_CAP(maxlen) char* text, size_t maxlen, SDL_PRINTF_FORMAT_STRING const char* fmt, ...)
{
...
static void SDLTest_PrintDisplayOrientation(
    char* text, size_t maxlen, SDL_DisplayOrientation orientation)
{
    switch (orientation) {
...
{
    const SDL_WindowFlags window_flags[] = { SDL_WINDOW_FULLSCREEN, SDL_WINDOW_OPENGL,
        SDL_WINDOW_OCCLUDED, SDL_WINDOW_HIDDEN, SDL_WINDOW_BORDERLESS, SDL_WINDOW_RESIZABLE,
        SDL_WINDOW_MINIMIZED, SDL_WINDOW_MAXIMIZED, SDL_WINDOW_MOUSE_GRABBED,
        SDL_WINDOW_INPUT_FOCUS, SDL_WINDOW_MOUSE_FOCUS, SDL_WINDOW_EXTERNAL, SDL_WINDOW_MODAL,
        SDL_WINDOW_HIGH_PIXEL_DENSITY, SDL_WINDOW_MOUSE_CAPTURE, SDL_WINDOW_MOUSE_RELATIVE_MODE,
        SDL_WINDOW_ALWAYS_ON_TOP, SDL_WINDOW_UTILITY, SDL_WINDOW_TOOLTIP, SDL_WINDOW_POPUP_MENU,
        SDL_WINDOW_KEYBOARD_GRABBED, SDL_WINDOW_VULKAN, SDL_WINDOW_METAL, SDL_WINDOW_TRANSPARENT,
...
{
    const SDL_Keymod kmod_flags[] = { SDL_KMOD_LSHIFT, SDL_KMOD_RSHIFT, SDL_KMOD_LCTRL,
        SDL_KMOD_RCTRL, SDL_KMOD_LALT, SDL_KMOD_RALT, SDL_KMOD_LGUI, SDL_KMOD_RGUI, SDL_KMOD_NUM,
        SDL_KMOD_CAPS, SDL_KMOD_MODE, SDL_KMOD_SCROLL };
...
static void SDLTest_PrintLogicalPresentation(
    char* text, size_t maxlen, SDL_RendererLogicalPresentation logical_presentation)
{
...
static void SDLTest_PrintRenderer(SDL_Renderer* renderer)
{
    ...
    SDL_Log("    VSync: %d\n",
        (int)SDL_GetNumberProperty(
            SDL_GetRendererProperties(renderer), SDL_PROP_RENDERER_VSYNC_NUMBER, 0));

    texture_formats = (const SDL_PixelFormat*)SDL_GetPointerProperty(
        SDL_GetRendererProperties(renderer), SDL_PROP_RENDERER_TEXTURE_FORMATS_POINTER, NULL);
    if (texture_formats) {
    ...
    max_texture_size = (int)SDL_GetNumberProperty(
...
static SDL_HitTestResult SDLCALL SDLTest_ExampleHitTestCallback(
...
bool SDLTest_CommonInit(SDLTest_CommonState* state)
{
        ...
        if (state->verbose & VERBOSE_MODES) {
            ...
            for (i = 0; i < n; ++i) {
                ...
                SDL_Log("  Desktop mode: %dx%d@%gx %gHz, %d bits-per-pixel (%s)\n", mode->w,
                    mode->h, mode->pixel_density, mode->refresh_rate, bpp,
...
#if defined(SDL_VIDEO_DRIVER_WINDOWS) && !defined(SDL_PLATFORM_XBOXONE)                            \
    && !defined(SDL_PLATFORM_XBOXSERIES)
...

        {
            ...
            SDL_GetClosestFullscreenDisplayMode(state->displayID, state->window_w, state->window_h,
                state->refresh_rate, include_high_density_modes, &state->fullscreen_mode);
        }

        state->windows = (SDL_Window**)SDL_calloc(state->num_windows, sizeof(*state->windows));
        state->renderers
            = (SDL_Renderer**)SDL_calloc(state->num_windows, sizeof(*state->renderers));
        ...
        for (i = 0; i < state->num_windows; ++i) {
            ...
            if (state->num_windows > 1) {
                (void)SDL_snprintf(
                    title, SDL_arraysize(title), "%s %d", state->window_title, i + 1);
            } else {
            ...
            if (state->window_min_aspect != 0.f || state->window_max_aspect != 0.f) {
                SDL_SetWindowAspectRatio(
                    state->windows[i], state->window_min_aspect, state->window_max_aspect);
            }
            ...
            if (!state->skip_renderer
                ...
                if (!SDL_SetRenderLogicalPresentation(state->renderers[i], state->logical_w,
                        state->logical_h, state->logical_presentation)) {
    ...
    if (state->flags & SDL_INIT_AUDIO) {
        ...
        const SDL_AudioSpec spec
            = { state->audio_format, state->audio_channels, state->audio_freq };
...
void SDLTest_PrintEvent(const SDL_Event* event)
{
    ...
    case SDL_EVENT_DISPLAY_ADDED:
        SDL_Log("SDL EVENT: Display %" SDL_PRIu32 " attached", event->display.displayID);
        break;
    ...
    case SDL_EVENT_DISPLAY_DESKTOP_MODE_CHANGED:
        SDL_Log("SDL EVENT: Display %" SDL_PRIu32 " desktop mode changed to %" SDL_PRIs32
                "x%" SDL_PRIs32,
            event->display.displayID, event->display.data1, event->display.data2);
        break;
    case SDL_EVENT_DISPLAY_CURRENT_MODE_CHANGED:
        SDL_Log("SDL EVENT: Display %" SDL_PRIu32 " current mode changed to %" SDL_PRIs32
                "x%" SDL_PRIs32,
            event->display.displayID, event->display.data1, event->display.data2);
        break;
    case SDL_EVENT_DISPLAY_MOVED:
        SDL_Log("SDL EVENT: Display %" SDL_PRIu32 " changed position", event->display.displayID);
        break;
    ...
    case SDL_EVENT_DISPLAY_REMOVED:
        SDL_Log("SDL EVENT: Display %" SDL_PRIu32 " removed", event->display.displayID);
        break;
    ...
    case SDL_EVENT_WINDOW_PIXEL_SIZE_CHANGED:
        SDL_Log("SDL EVENT: Window %" SDL_PRIu32 " changed pixel size to %" SDL_PRIs32
                "x%" SDL_PRIs32,
            event->window.windowID, event->window.data1, event->window.data2);
        break;
    case SDL_EVENT_WINDOW_METAL_VIEW_RESIZED:
        SDL_Log(
            "SDL EVENT: Window %" SDL_PRIu32 " changed metal view size", event->window.windowID);
        break;
    ...
    case SDL_EVENT_WINDOW_DISPLAY_CHANGED:
        SDL_Log("SDL EVENT: Window %" SDL_PRIu32 " display changed to %" SDL_PRIs32,
            event->window.windowID, event->window.data1);
        break;
    case SDL_EVENT_WINDOW_DISPLAY_SCALE_CHANGED:
        SDL_Log("SDL EVENT: Window %" SDL_PRIu32 " display scale changed to %d%%",
            event->window.windowID,
            (int)(SDL_GetWindowDisplayScale(SDL_GetWindowFromEvent(event)) * 100.0f));
        break;
    ...
    case SDL_EVENT_WINDOW_HDR_STATE_CHANGED:
        SDL_Log("SDL EVENT: Window %" SDL_PRIu32 " HDR %s", event->window.windowID,
            event->window.data1 ? "enabled" : "disabled");
        break;
    case SDL_EVENT_KEYBOARD_ADDED:
        SDL_Log("SDL EVENT: Keyboard %" SDL_PRIu32 " attached", event->kdevice.which);
        break;
    case SDL_EVENT_KEYBOARD_REMOVED:
        SDL_Log("SDL EVENT: Keyboard %" SDL_PRIu32 " removed", event->kdevice.which);
        break;
    ...
    case SDL_EVENT_KEY_UP: {
        ...
        SDL_Log("SDL EVENT: Keyboard: key %s in window %" SDL_PRIu32
                ": scancode 0x%08X = %s, keycode 0x%08" SDL_PRIX32 " = %s, mods = %s",
            (event->type == SDL_EVENT_KEY_DOWN) ? "pressed" : "released", event->key.windowID,
            event->key.scancode, SDL_GetScancodeName(event->key.scancode), event->key.key,
            SDL_GetKeyName(event->key.key), modstr);
        break;
    }
    case SDL_EVENT_TEXT_EDITING:
        SDL_Log("SDL EVENT: Keyboard: text editing \"%s\" in window %" SDL_PRIu32, event->edit.text,
            event->edit.windowID);
        break;
    ...
    case SDL_EVENT_TEXT_INPUT:
        SDL_Log("SDL EVENT: Keyboard: text input \"%s\" in window %" SDL_PRIu32, event->text.text,
            event->text.windowID);
        break;
    ...
    case SDL_EVENT_MOUSE_ADDED:
        SDL_Log("SDL EVENT: Mouse %" SDL_PRIu32 " attached", event->mdevice.which);
        break;
    case SDL_EVENT_MOUSE_REMOVED:
        SDL_Log("SDL EVENT: Mouse %" SDL_PRIu32 " removed", event->mdevice.which);
        break;
    case SDL_EVENT_MOUSE_MOTION:
        SDL_Log("SDL EVENT: Mouse: moved to %g,%g (%g,%g) in window %" SDL_PRIu32, event->motion.x,
            event->motion.y, event->motion.xrel, event->motion.yrel, event->motion.windowID);
        break;
    case SDL_EVENT_MOUSE_BUTTON_DOWN:
        SDL_Log("SDL EVENT: Mouse: button %d pressed at %g,%g with click count %d in window "
                "%" SDL_PRIu32,
            event->button.button, event->button.x, event->button.y, event->button.clicks,
            event->button.windowID);
        break;
    case SDL_EVENT_MOUSE_BUTTON_UP:
        SDL_Log("SDL EVENT: Mouse: button %d released at %g,%g with click count %d in window "
                "%" SDL_PRIu32,
            event->button.button, event->button.x, event->button.y, event->button.clicks,
            event->button.windowID);
        break;
    case SDL_EVENT_MOUSE_WHEEL:
        SDL_Log("SDL EVENT: Mouse: wheel scrolled %g in x and %g in y (reversed: %d) in window "
                "%" SDL_PRIu32,
            event->wheel.x, event->wheel.y, event->wheel.direction, event->wheel.windowID);
        break;
    case SDL_EVENT_JOYSTICK_ADDED:
        SDL_Log("SDL EVENT: Joystick %" SDL_PRIu32 " attached", event->jdevice.which);
        break;
    case SDL_EVENT_JOYSTICK_REMOVED:
        SDL_Log("SDL EVENT: Joystick %" SDL_PRIu32 " removed", event->jdevice.which);
        break;
    case SDL_EVENT_JOYSTICK_AXIS_MOTION:
        SDL_Log("SDL EVENT: Joystick %" SDL_PRIu32 " axis %d value: %d", event->jaxis.which,
            event->jaxis.axis, event->jaxis.value);
        break;
    case SDL_EVENT_JOYSTICK_BALL_MOTION:
        SDL_Log("SDL EVENT: Joystick %" SDL_PRIs32 ": ball %d moved by %d,%d", event->jball.which,
    ...
    case SDL_EVENT_JOYSTICK_HAT_MOTION: {
        ...
        SDL_Log("SDL EVENT: Joystick %" SDL_PRIu32 ": hat %d moved to %s", event->jhat.which,
            event->jhat.hat, position);
    } break;
    case SDL_EVENT_JOYSTICK_BUTTON_DOWN:
        SDL_Log("SDL EVENT: Joystick %" SDL_PRIu32 ": button %d pressed", event->jbutton.which,
            event->jbutton.button);
        break;
    case SDL_EVENT_JOYSTICK_BUTTON_UP:
        SDL_Log("SDL EVENT: Joystick %" SDL_PRIu32 ": button %d released", event->jbutton.which,
            event->jbutton.button);
        break;
    case SDL_EVENT_JOYSTICK_BATTERY_UPDATED:
        SDL_Log("SDL EVENT: Joystick %" SDL_PRIu32 ": battery at %d percent", event->jbattery.which,
            event->jbattery.percent);
        break;
    case SDL_EVENT_GAMEPAD_ADDED:
        SDL_Log("SDL EVENT: Gamepad %" SDL_PRIu32 " attached", event->gdevice.which);
        break;
    case SDL_EVENT_GAMEPAD_REMOVED:
        SDL_Log("SDL EVENT: Gamepad %" SDL_PRIu32 " removed", event->gdevice.which);
        break;
    case SDL_EVENT_GAMEPAD_REMAPPED:
        SDL_Log("SDL EVENT: Gamepad %" SDL_PRIu32 " mapping changed", event->gdevice.which);
        break;
    case SDL_EVENT_GAMEPAD_AXIS_MOTION:
        SDL_Log("SDL EVENT: Gamepad %" SDL_PRIu32 " axis %d ('%s') value: %d", event->gaxis.which,
            event->gaxis.axis, GamepadAxisName((SDL_GamepadAxis)event->gaxis.axis),
            event->gaxis.value);
        break;
    case SDL_EVENT_GAMEPAD_BUTTON_DOWN:
        SDL_Log("SDL EVENT: Gamepad %" SDL_PRIu32 "button %d ('%s') down", event->gbutton.which,
            event->gbutton.button, GamepadButtonName((SDL_GamepadButton)event->gbutton.button));
        break;
    case SDL_EVENT_GAMEPAD_BUTTON_UP:
        SDL_Log("SDL EVENT: Gamepad %" SDL_PRIu32 " button %d ('%s') up", event->gbutton.which,
            event->gbutton.button, GamepadButtonName((SDL_GamepadButton)event->gbutton.button));
        break;
    ...
    case SDL_EVENT_FINGER_MOTION:
        SDL_Log("SDL EVENT: Finger: motion touch=%" SDL_PRIu64 ", finger=%" SDL_PRIu64
    ...
    case SDL_EVENT_FINGER_UP:
        SDL_Log("SDL EVENT: Finger: %s touch=%" SDL_PRIu64 ", finger=%" SDL_PRIu64
                ", x=%f, y=%f, dx=%f, dy=%f, pressure=%f",
            (event->type == SDL_EVENT_FINGER_DOWN) ? "down" : "up", event->tfinger.touchID,
            event->tfinger.fingerID, event->tfinger.x, event->tfinger.y, event->tfinger.dx,
    ...
    case SDL_EVENT_DROP_POSITION:
        SDL_Log("SDL EVENT: Drag and drop moving in window %" SDL_PRIu32 ": %g,%g",
            event->drop.windowID, event->drop.x, event->drop.y);
        break;
    case SDL_EVENT_DROP_FILE:
        SDL_Log("SDL EVENT: Drag and drop file in window %" SDL_PRIu32 ": '%s'",
            event->drop.windowID, event->drop.data);
        break;
    case SDL_EVENT_DROP_TEXT:
        SDL_Log("SDL EVENT: Drag and drop text in window %" SDL_PRIu32 ": '%s'",
            event->drop.windowID, event->drop.data);
        break;
    ...
    case SDL_EVENT_AUDIO_DEVICE_ADDED:
        SDL_Log("SDL EVENT: Audio %s device %" SDL_PRIu32 " available",
            event->adevice.recording ? "recording" : "playback", event->adevice.which);
        break;
    ...
    case SDL_EVENT_CAMERA_DEVICE_ADDED:
        SDL_Log("SDL EVENT: Camera device %" SDL_PRIu32 " available", event->cdevice.which);
        break;
    case SDL_EVENT_CAMERA_DEVICE_REMOVED:
        SDL_Log("SDL EVENT: Camera device %" SDL_PRIu32 " removed", event->cdevice.which);
        break;
    case SDL_EVENT_CAMERA_DEVICE_APPROVED:
        SDL_Log(
            "SDL EVENT: Camera device %" SDL_PRIu32 " permission granted", event->cdevice.which);
        break;
    case SDL_EVENT_CAMERA_DEVICE_DENIED:
        SDL_Log("SDL EVENT: Camera device %" SDL_PRIu32 " permission denied", event->cdevice.which);
        break;
    case SDL_EVENT_SENSOR_UPDATE:
        SDL_Log("SDL EVENT: Sensor update for %" SDL_PRIu32, event->sensor.which);
        break;
    case SDL_EVENT_PEN_PROXIMITY_IN:
        SDL_Log("SDL EVENT: Pen %" SDL_PRIu32 " entered proximity", event->pproximity.which);
        break;
    case SDL_EVENT_PEN_PROXIMITY_OUT:
        SDL_Log("SDL EVENT: Pen %" SDL_PRIu32 " left proximity", event->ptouch.which);
        break;
    case SDL_EVENT_PEN_DOWN:
        SDL_Log("SDL EVENT: Pen %" SDL_PRIu32 " touched down at %g,%g", event->ptouch.which,
            event->ptouch.x, event->ptouch.y);
        break;
    case SDL_EVENT_PEN_UP:
        SDL_Log("SDL EVENT: Pen %" SDL_PRIu32 " lifted off at %g,%g", event->ptouch.which,
            event->ptouch.x, event->ptouch.y);
        break;
    case SDL_EVENT_PEN_BUTTON_DOWN:
        SDL_Log("SDL EVENT: Pen %" SDL_PRIu32 " button %d pressed at %g,%g", event->pbutton.which,
            event->pbutton.button, event->pbutton.x, event->pbutton.y);
        break;
    case SDL_EVENT_PEN_BUTTON_UP:
        SDL_Log("SDL EVENT: Pen %" SDL_PRIu32 " button %d released at %g,%g", event->pbutton.which,
            event->pbutton.button, event->pbutton.x, event->pbutton.y);
        break;
    case SDL_EVENT_PEN_MOTION:
        SDL_Log("SDL EVENT: Pen %" SDL_PRIu32 " moved to %g,%g", event->pmotion.which,
            event->pmotion.x, event->pmotion.y);
        break;
    case SDL_EVENT_PEN_AXIS:
        SDL_Log("SDL EVENT: Pen %" SDL_PRIu32 " axis %d changed to %.2f", event->paxis.which,
            event->paxis.axis, event->paxis.value);
        break;
...
static const void* SDLCALL SDLTest_ScreenShotClipboardProvider(
    void* context, const char* mime_type, size_t* size)
{
    SDLTest_ClipboardData* data = (SDLTest_ClipboardData*)context;
...
{
    SDL_Surface* surface;
    const char* image_formats[] = { "text/plain;charset=utf-8", "image/bmp" };
    ...
    SDL_SetClipboardData(SDLTest_ScreenShotClipboardProvider, SDLTest_ScreenShotClipboardCleanup,
        clipboard_data, image_formats, SDL_arraysize(image_formats));
        ...
        if (window) {
            ...
            if (mode) {
                ...
                if (!SDL_SetWindowFullscreenMode(window, &new_mode)) {
                    ...
                    if (SDL_GetClosestFullscreenDisplayMode(displays[index], state->window_w,
                            state->window_h, state->refresh_rate, include_high_density_modes,
                            &new_mode)) {
...
SDL_AppResult SDLTest_CommonEventMainCallbacks(SDLTest_CommonState* state, const SDL_Event* event)
{
    ...
    if (state->verbose & VERBOSE_EVENT) {
        if ((event->type != SDL_EVENT_MOUSE_MOTION && event->type != SDL_EVENT_FINGER_MOTION
                && event->type != SDL_EVENT_PEN_MOTION && event->type != SDL_EVENT_PEN_AXIS
    ...
    case SDL_EVENT_KEY_DOWN: {
                ...
                if (window) {
                    ...
                    if (displays) {
                        ...
                        if (current_index >= 0) {
                            ...
                            SDL_SetWindowPosition(window, SDL_WINDOWPOS_CENTERED_DISPLAY(dest),
        ...
        case SDLK_RETURN:
            if (withControl) {
                ...
                if (window) {
                    SDL_WindowFlags flags = SDL_GetWindowFlags(window);
                    if (!(flags & SDL_WINDOW_FULLSCREEN) || !SDL_GetWindowFullscreenMode(window)) {
        ...
        case SDLK_0:
            if (withControl) {
                ...
                SDL_ShowSimpleMessageBox(
                    SDL_MESSAGEBOX_INFORMATION, "Test Message", "You're awesome!", window);
            }
            break;
        case SDLK_1:
            if (withControl) {
...
void SDLTest_CommonDrawWindowInfo(SDL_Renderer* renderer, SDL_Window* window, float* usedHeight)
{
    ...
    (void)SDL_snprintf(
        text, sizeof(text), "SDL_GetCurrentVideoDriver: %s", SDL_GetCurrentVideoDriver());
    ...
    (void)SDL_snprintf(
        text, sizeof(text), "SDL_GetRenderViewport: %d,%d, %dx%d", rect.x, rect.y, rect.w, rect.h);
    SDLTest_DrawString(renderer, 0.0f, textY, text);
    textY += lineHeight;

    SDL_GetRenderScale(renderer, &scaleX, &scaleY);
    (void)SDL_snprintf(text, sizeof(text), "SDL_GetRenderScale: %g,%g", scaleX, scaleY);
    SDLTest_DrawString(renderer, 0.0f, textY, text);
    textY += lineHeight;
    ...
    (void)SDL_snprintf(
        text, sizeof(text), "SDL_GetWindowSafeArea: %d,%d %dx%d", rect.x, rect.y, rect.w, rect.h);
    SDLTest_DrawString(renderer, 0.0f, textY, text);
    textY += lineHeight;
    ...
    (void)SDL_snprintf(
        text, sizeof(text), "SDL_GetDisplayForWindow: %" SDL_PRIu32, windowDisplayID);
    SDLTest_DrawString(renderer, 0.0f, textY, text);
    textY += lineHeight;

    (void)SDL_snprintf(
        text, sizeof(text), "SDL_GetDisplayName: %s", SDL_GetDisplayName(windowDisplayID));
    SDLTest_DrawString(renderer, 0.0f, textY, text);
    textY += lineHeight;

    if (SDL_GetDisplayBounds(windowDisplayID, &rect)) {
        (void)SDL_snprintf(text, sizeof(text), "SDL_GetDisplayBounds: %d,%d, %dx%d", rect.x, rect.y,
            rect.w, rect.h);
        SDLTest_DrawString(renderer, 0.0f, textY, text);
        textY += lineHeight;
    }
    ...
    SDLTest_PrintDisplayOrientation(
        text, sizeof(text), SDL_GetNaturalDisplayOrientation(windowDisplayID));
    SDLTest_DrawString(renderer, 0.0f, textY, text);
    textY += lineHeight;

    (void)SDL_snprintf(text, sizeof(text), "SDL_GetCurrentDisplayOrientation: ");
    SDLTest_PrintDisplayOrientation(
        text, sizeof(text), SDL_GetCurrentDisplayOrientation(windowDisplayID));
    SDLTest_DrawString(renderer, 0.0f, textY, text);
    textY += lineHeight;
