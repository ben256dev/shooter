...
u0 vec2_add_vec2(vec2* p_result, vec2 a, vec2 b)
{
    ...
    // Store the sum
    *p_result = (vec2) { .x = a.x + b.x, .y = a.y + b.y };
...
u0 vec2_to_vec4(vec4* p_result, vec2 v)
{
    ...
    *p_result = (vec4) { .x = v.x, .y = v.y, .z = 0, .w = 0 };
...
u0 vec3_add_vec3(vec3* p_result, vec3 a, vec3 b)
{
    ...
    // Store the sum
    *p_result = (vec3) { .x = a.x + b.x, .y = a.y + b.y, .z = a.z + b.z };
...
u0 vec3_mul_scalar(vec3* p_result, vec3 v, float s)
{
    ...
    // Store the scaled vector
    *p_result = (vec3) { .x = v.x * s, .y = v.y * s, .z = v.z * s };
...
u0 vec3_to_vec4(vec4* p_result, vec3 v)
{
    ...
    *p_result = (vec4) { .x = v.x, .y = v.y, .z = v.z, .w = 0 };
...
u0 vec3_cross_product(vec3* p_result, vec3 a, vec3 b)
{
    ...
    *p_result = (vec3) { a.y * b.z - a.z * b.y, a.z * b.x - a.x * b.z, a.x * b.y - a.y * b.x };
...
u0 vec3_normalize(vec3* p_result, vec3 v)
{
    ...
    *p_result = (vec3) { .x = v.x / l, .y = v.y / l, .z = v.z / l };
...
{
    // Store the sum
    *p_result = (vec4) { .x = a.x + b.x, .y = a.y + b.y, .z = a.z + b.z, .w = a.w + b.w };
...
u0 vec4_to_vec3(vec3* p_result, vec4 v)
{
    ...
    *p_result = (vec3) { .x = v.x, .y = v.y, .z = v.z };
...
u0 mat2_mul_vec2(vec2* p_result, mat2 m, vec2 v)
{
    ...
    // Store the product
    *p_result = (vec2) { m.a * v.x + m.b * v.y, m.c * v.x + m.d * v.y };
...
u0 mat2_mul_mat2(mat2* p_result, mat2 m, mat2 n)
{
    ...
    // Store the product
    *p_result = (mat2) { .a = m.a * n.a + m.b * n.c,
        .b = m.a * n.b + m.b * n.d,
        .c = m.c * n.a + m.d * n.c,
        .d = m.c * n.b + m.d * n.d };
...
u0 mat2_transpose(mat2* p_result, mat2 m)
{
    ...
    // Store the transpose
    *p_result = (mat2) { .a = m.a, .b = m.c, .c = m.b, .d = m.d };
...
u0 mat2_identity(mat2* p_result)
{
    ...
    *p_result = (mat2) { .a = 1.f, .b = 0.f, .c = 0.f, .d = 1.f };
...
u0 mat2_to_mat3(mat3* p_result, mat2 m)
{
    ...
    *p_result
        = (mat3) { .a = m.a, .b = m.b, .c = 0, .d = m.c, .e = m.d, .f = 0, .g = 0, .h = 0, .i = 1 };
...
u0 mat3_mul_vec3(vec3* p_result, mat3 m, vec3 v)
{
    ...
    // Store the product
    *p_result = (vec3) { .x = m.a * v.x + m.b * v.y + m.c * v.z,
...
u0 mat3_mul_mat3(mat3* p_result, mat3 m, mat3 n)
{
    ...
    // Store the product
    *p_result = (mat3) {
        .a = (m.a * n.a + m.b * n.d + m.c * n.g),
        .b = (m.a * n.b + m.b * n.e + m.c * n.h),
        .c = (m.a * n.c + m.b * n.f + m.c * n.i),
        .d = (m.d * n.a + m.e * n.d + m.f * n.g),
        .e = (m.d * n.b + m.e * n.e + m.f * n.h),
        .f = (m.d * n.c + m.e * n.f + m.f * n.i),
        .g = (m.g * n.a + m.h * n.d + m.i * n.g),
        .h = (m.g * n.b + m.h * n.e + m.i * n.h),
        .i = (m.g * n.c + m.h * n.f + m.i * n.i),
    };
...
u0 mat3_identity(mat3* p_result)
{
    ...
    // Store the identity matrix
    *p_result = (mat3) {
        .a = 1.f, .b = 0.f, .c = 0.f, .d = 0.f, .e = 1.f, .f = 0.f, .g = 0.f, .h = 0.f, .i = 1.f
    };
...
u0 mat3_to_mat2(mat2* p_result, mat3 m)
{
    ...
    // Store the mat2 as a mat3
    *p_result = (mat2) { .a = m.a, .b = m.b, .c = m.d, .d = m.e };
...
u0 mat4_mul_vec4(vec4* p_result, mat4 m, vec4 v)
{
    ...
    // Store the product
    *p_result = (vec4) { .x = m.a * v.x + m.b * v.y + m.c * v.z + m.d * v.w,
...
u0 mat4_mul_mat4(mat4* p_result, mat4 m, mat4 n)
{
    ...
    // Store the product
    *p_result = (mat4) { .a = (m.a * n.a + m.b * n.e + m.c * n.i + m.d * n.m),
        .b = (m.a * n.b + m.b * n.f + m.c * n.j + m.d * n.n),
        .c = (m.a * n.c + m.b * n.g + m.c * n.k + m.d * n.o),
        .d = (m.a * n.d + m.b * n.h + m.c * n.l + m.d * n.p),
        .e = (m.e * n.a + m.f * n.e + m.g * n.i + m.h * n.m),
        .f = (m.e * n.b + m.f * n.f + m.g * n.j + m.h * n.n),
        .g = (m.e * n.c + m.f * n.g + m.g * n.k + m.h * n.o),
        .h = (m.e * n.d + m.f * n.h + m.g * n.l + m.h * n.p),
        .i = (m.i * n.a + m.j * n.e + m.k * n.i + m.l * n.m),
        .j = (m.i * n.b + m.j * n.f + m.k * n.j + m.l * n.n),
        .k = (m.i * n.c + m.j * n.g + m.k * n.k + m.l * n.o),
        .l = (m.i * n.d + m.j * n.h + m.k * n.l + m.l * n.p),
        .m = (m.m * n.a + m.n * n.e + m.o * n.i + m.p * n.m),
        .n = (m.m * n.b + m.n * n.f + m.o * n.j + m.p * n.n),
        .o = (m.m * n.c + m.n * n.g + m.o * n.k + m.p * n.o),
        .p = (m.m * n.d + m.n * n.h + m.o * n.l + m.p * n.p) };
...
u0 mat4_to_mat2(mat2* p_result, mat4 m)
{
    ...
    *p_result = (mat2) { .a = m.a, .b = m.b, .c = m.e, .d = m.f };
...
u0 mat4_to_mat3(mat3* p_result, mat4 m)
{
    ...
    *p_result = (mat3) {
        .a = m.a, .b = m.b, .c = m.c, .d = m.e, .e = m.f, .f = m.g, .g = m.i, .h = m.j, .i = m.k
    };
...
u0 mat4_model_from_vec3(mat4* p_result, vec3 location, vec3 rotation, vec3 scale)
{
    ...
    // Uninitialized data
    mat4 _location, _rotation, _scale, _rotation_scale;
...
u0 mat4_view_from_vec3(mat4* p_view, vec3 eye, vec3 target, vec3 up)
{
    ...
    // Initialized data
    vec3 _forward = { 0 }, _left = { 0 }, _up = { 0 }, _scratch = { 0 };

    // Compute the forward vector
    vec3_normalize(
        &_forward, (vec3) { .x = eye.x - target.x, .y = eye.y - target.y, .z = eye.z - target.z });
...
u0 mat4_perspective_from_vec3(
    mat4* p_projection, float fov, float aspect, float near_clip, float far_clip)
{

    // Argument check
    if (p_projection == (void*)0)
        goto no_projection;
    ...
    *p_projection = (mat4) {
        ...
        .i = 0,
        .j = 0,
        .k = -(far_clip + near_clip) / (far_clip - near_clip),
        .l = -1,
        .m = 0,
        .n = 0,
        .o = 2 * far_clip * near_clip / (near_clip - far_clip),
        .p = 0,
    };
